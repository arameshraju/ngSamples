...
(function(window, document, $, angular) {
        ...
        var DTOptions = {
            ...
            withPaginationType: function(sPaginationType) {
                if (angular.isString(sPaginationType)) {
                    ...
                    throw new Error('The pagination type must be prov"id"ed');
                }
                return this;
            },
             ...
             * @param "id"isplayLength the number of items per page
             * @returns {DTOptions} the options
             */
            withDisplayLength: function("id"isplayLength) {
                this."id"isplayLength = "id"isplayLength;
                return this;
            },
    ...
    function dtColumnBuilder() {
        ...
        var DTColumn = {
             ...
             * @param s"class" the CSS "class"
             * @returns {DTColumn} the wrapped datatables column
             */
            with"class": function(s"class") {
                this.s"class" = s"class";
                return this;
            },
    ...
    function dtLoadingTemplate($compile, DTDefaultOptions, DT_LOADING_"class") {
        return {
            compileHtml: function($scope) {
                return $compile(angular.element('<div "class"="' + DT_LOADING_"class" + '">' + DTDefaultOptions.loadingTemplate + '</div>'))($scope);
            },
            isLoading: function(elem) {
                return elem.has"class"(DT_LOADING_"class");
            }
        };
    }
    dtLoadingTemplate.$inject = ['$compile', 'DTDefaultOptions', 'DT_LOADING_"class"'];
        ...
        if ($.fn.DataTable.Api) {
            ...
            $.fn.DataTable.Api.register('ngDestroy()', function(remove) {
                ...
                return this.iterator('table', function(settings) {
                    ...
                    $.fn.DataTable.ext.internal._fnSorting"class"es(settings);
                    ...
                    var "id"x = $.inArray(settings, $.fn.DataTable.settings);
                    if ("id"x !== -1) {
                        $.fn.DataTable.settings.splice("id"x, 1);
                    }
                });
            });
        }
    ...
    angular.module('datatables.options', [])
        .constant('DT_DEFAULT_OPTIONS', {
            ...
            sAjaxDataProp: '',
            // Set default columns (used when none are prov"id"ed)
            aoColumns: []
        })
        .constant('DT_LOADING_"class"', 'dt-loading')
    ...
    function dtRendererService(DTLoadingTemplate) {
        ...
        var rendererService = {
            ...
            h"id"eLoading: h"id"eLoading,
            renderDataTable: renderDataTable,
            h"id"eLoadingAndRenderDataTable: h"id"eLoadingAndRenderDataTable,
        ...
        function renderDataTable($elem, options) {
            var dt"id" = '#' + $elem.attr('"id"');
            if ($.fn.dataTable.isDataTable(dt"id") && angular.isObject(options)) {
        ...
        function h"id"eLoadingAndRenderDataTable($elem, options) {
            rendererService.h"id"eLoading($elem);
    ...
    function dtDefaultRenderer($q, DTRenderer, DTRendererService, DTInstanceFactory) {
        ...
        function create(options) {
            ...
            function render($elem, $scope) {
                ...
                var result = DTRendererService.h"id"eLoadingAndRenderDataTable($elem, renderer.options);
    ...
    function dtNGRenderer($log, $q, $compile, $timeout, DTRenderer, DTRendererService, DTInstanceFactory) {
        ...
        function create(options) {
            ...
            function render($elem, $scope, staticHTML) {
                ...
                if (!_match) {
                    throw new Error('Expected expression in form of "_item_ in _collection_[ track by _"id"_]" but got "{0}".', _expression);
                }
                ...
                _parentScope.$watchCollection(_ngRepeatAttr, function() {
                    ...
                    $timeout(function() {
                        ...
                        var result = DTRendererService.h"id"eLoadingAndRenderDataTable(_$elem, renderer.options);
                        _oTable = result.DataTable;
                        DTInstanceFactory.copyDTProperties(result, dtInstance);
        ...
        function create(options) {
            ...
            function reloadData(callback, resetPaging) {
                ...
                if (angular.isFunction(renderer.options.fnPromise)) {
                    ...
                    $log.warn('In order to use the reloadData functionality with a Promise renderer, you need to prov"id"e a function that returns a promise.');
                }
            }
            ...
            function _resolve(fnPromise, callback) {
                ...
                if (angular.isUndefined(fnPromise)) {
                    throw new Error('You must prov"id"e a promise or a function that returns a promise!');
                }
                if (_loadedPromise) {
            ...
            function _doRender(options, $elem, data, callback) {
                ...
                // By removing the $promise attribute, we avo"id" an infinite loop when jquery is cloning the data
                // See https://github.com/l-lin/angular-datatables/issues/110
                delete data.$promise;
                options.aaData = data;
                // Add $timeout to be sure that angular has finished rendering before calling datatables
                $timeout(function() {
                    DTRendererService.h"id"eLoading($elem);
    ...
    function dtRendererFactory(DTDefaultRenderer, DTNGRenderer, DTPromiseRenderer, DTAjaxRenderer) {
        ...
        function fromOptions(options, isNgDisplay) {
            if (isNgDisplay) {
                if (options && options.serverS"id"e) {
                    throw new Error('You cannot use server s"id"e processing along with the Angular renderer!');
                }
                return DTNGRenderer.create(options);
            }
            if (angular.isDefined(options)) {
                if (angular.isDefined(options.fnPromise) && options.fnPromise !== null) {
                    if (options.serverS"id"e) {
                        throw new Error('You cannot use server s"id"e processing along with the Promise renderer!');
                    }
    ...
    function dtPropertyUtil($q) {
        return {
            overr"id"eProperties: overr"id"eProperties,
        ...
        /**
         * Overr"id"es the source property with the given target properties.
         * Source is not written. It's making a fresh copy of it in order to ensure that we do not change the parameters.
         * @param source the source properties to overr"id"e
         * @param target the target properties
         * @returns {*} the object overr"id"ed
         */
        function overr"id"eProperties(source, target) {
            ...
            if (angular.isObject(target)) {
                for (var prop in target) {
                    if (target.hasOwnProperty(prop)) {
                        result[prop] = overr"id"eProperties(result[prop], target[prop]);
                    }
                }
            } else {
         ...
         */
        function deleteProperty(obj, property"name") {
         ...
         * @param obj the object
         * @param excludedProperties"name" the list of properties to ignore
         * @returns {promise} the promise that the object attributes promises are all resolved
         */
        function resolveObjectPromises(obj, excludedProperties"name") {
            var defer = $q.defer(),
                promises = [],
                resolvedObj = {},
                excludedProp = excludedProperties"name" || [];
